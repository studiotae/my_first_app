...
第1条（本アプリの目的と性質）
1. 個人利用の原則
本アプリは、ユーザー自身が所有する学習資料（ノート、配布プリント等）を整理・管理するための個人利用専用ツールです。
2. 共有機能の不存在
...
2. 利用許諾
ユーザーは、運営者に対し、本サービスを提供および維持・改善するために必要な範囲（データの変換、一時的な保存、サムネイル表示、バックアップ等を含みますがこれらに限りません）において、コンテンツを使用、複製、翻案、および公衆送信するための、非独占的かつ無償の権利を許諾するものとします。
3. 権利の保証
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _startAnalysis(List<File> files) async {
    ...
    final analyzer = DocumentAnalyzer(apiKey: _apiKey);
    String? lastResult; 
